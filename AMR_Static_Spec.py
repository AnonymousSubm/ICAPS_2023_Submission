...
def main():
    ...
    constraints_of_action_generation = [
        ...
        #E4 error handling
        "forall p. at(p) and E4 implies M(p)"
    ]
    ...
    action_specification = {
        ...
        #"move5": "forall x,y. move5(x,y) and at(x) implies at(y) and not at(x) and not docked(x) and not assigned(x) and released(x) ",
    ...
    domain = {"D1": ["1", "2", "3"], "D2": ["1", "2", "3", "4", "5", "6", "7", "8", "8", "9", "10"],
    ...
    for key in properties:
        p=properties[key]
        for i in p:
            if 'safe1' not in p[i] or 'safe2' not in p[i]:
                print(key,i,p[i])
